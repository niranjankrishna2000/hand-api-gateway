...
func (c *Client) generatePdf(invoice Invoice) error {
	...
	err = drawCell(table, c.newPara("Payment Invoice", headerStyle), cellStyles["centered"])
	if err != nil {
		return err
	}
...
func (c *Client) writeInvoice(invoice Invoice) error {
	log.Println("Starting to Write Invoice")
	headerStyle := c.creator.NewTextStyle()
	// Invoice Header info table.
	table := c.creator.NewTable(2)
	table.SetMargins(0, 0, 50, 0)
	err := drawCell(table, c.newPara("Name: "+invoice.Name, headerStyle), cellStyles["left"])
	if err != nil {
		return err
	}
	err = drawCell(table, c.newPara("Paid Address: "+invoice.Address, headerStyle), cellStyles["left"])
	if err != nil {
		return err
	}
	err = drawCell(table, c.newPara("Invoice Id: "+invoice.InvoiceID, headerStyle), cellStyles["left"])
	if err != nil {
		return err
	}
	err = drawCell(table, c.newPara("GSTIN: 12AAAAAAAA0A1Z5", headerStyle), cellStyles["left"])
	if err != nil {
		return err
	}
	err = c.creator.Draw(table)
